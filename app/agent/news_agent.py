...
class NewsAnalysis(BaseModel):
    ...
    resumen: str = Field(...,
                         description="Resumen detallado de la noticia (neutro)")
    analisis: str = Field(..., description="Análisis financiero explicándole al usuario por qué esto le importa (en segunda persona, ej: 'Esta noticia te afecta porque...')")
    ...
    fuente_url: Optional[str] = Field(
        None, description="URL de la fuente original")
...
- NO usa Agno (llamada directa a API Nvidia con openai)
    ...
    CATEGORY_KEYWORDS = {
        "Combustible": ["bencina", "gasolina", "combustible", "petróleo", "diésel", "gas", "auto", "vehículo", "camión", "transporte público", "taxi", "uber"],
        "Vivienda": ["arriendo", "renta", "alquiler", "departamento", "casa", "vivienda", "hipoteca", "dividendo", "inmobiliar", "propiedad", "corredora"],
        "Alimentos": ["alimento", "supermercado", "precio", "inflación", "carne", "pan", "comida", "verdura", "fruta", "canasta", "aceite", "leche"],
        "Servicios básicos": ["luz", "agua", "internet", "electricidad", "calefacción", "gas domiciliario", "basura", "alcantarillado", "cuenta", "suministro"],
        "Impuestos": ["impuesto", "tributario", "sii", "renta", "iva", "declaración", "reforma tributaria", "exención"],
        "Créditos": ["crédito", "deuda", "tarjeta", "banco", "préstamo", "financiamiento", "cobranza", "tasa", "cmf", "morosidad"],
        "Ahorro": ["ahorro", "inversión", "fondo", "depósito", "plazo fijo", "rendimiento", "interés", "cuenta"],
        "Sueldo mínimo": ["sueldo", "salario", "ingreso", "trabajo", "empleo", "sindicato", "huelga", "negociación", "salario mínimo", "remuneración"],
        "Transporte": ["transporte", "bus", "metro", "tren", "movilidad", "pasaje", "tarifa", "subsidio transporte"],
        "Pensiones": ["pensión", "jubilación", "afp", "vejez", "jubilado", "retiro", "previsión", "ahorro previsional"],
        "Mercado accionario": ["bolsa", "acción", "índice", "mercado bursátil", "ip", "valor"],
        "Criptomonedas": ["cripto", "bitcoin", "ethereum", "blockchain", "crypto", "activo digital"],
        "Política económica": ["política económica", "gobierno", "ministerio", "reforma", "proyecto de ley", "banco central", "hacienda", "economía"],
    }
    ...
    def _build_user_context(self, user_profile: dict) -> str:
        ...
        fs = "estable" if mb > 0 and dl == "bajo" else "moderado" if mb > 0 else "inestable"
        it = user_profile.get("income_type_rel", {}).get(
    ...
    async def get_latest_rss_news(self, limit: int = 10) -> list[dict]:
        ...
        for feed in feeds:
            ...
            source = feed_meta.get("title", "desconocida") if hasattr(
                feed_meta, "get") else "desconocida"
    ...
    async def get_latest_news_from_endpoint(self, limit: int = 15) -> list[dict]:
        ...
        import httpx
        endpoint = os.getenv("NEWS_ENDPOINT_URL",
                             "http://localhost:8000/v1/news/latest_news")
        ...
        try:
            ...
            items = data if isinstance(
                data, list) else data.get("analyses", [])
            formatted = []
            for item in items:
                url = item.get("source_url") or item.get(
                    "fuente_url") or item.get("link", "")
                resolved = await self._resolve_google_news_url(url)
    ...
    async def search_chilean_news(self, user_categories: list[str], keywords: str = "") -> list[dict]:
        ...
        if not tavily_key:
            logger.warning(
                "TAVILY_API_KEY no configurada, saltando búsqueda chilena")
            return []
        try:
            ...
            resp = tc.search(query=query, search_depth="advanced",
            ...
            for r in resp.get("results", []):
                ...
                if url:
                    results.append({
                        "title": r.get("title", "").strip() or url.split("/")[-1][:50],
    ...
    async def _analyze_batch(self, batch: list[dict], user_context: str, batch_idx: int) -> Optional[list[dict]]:
        ...
        news_json_example = ",\n".join([
            ...
            }, ensure_ascii=False) for _ in range(len(batch))
        ])
        ...
        for client_idx, client in enumerate(self.clients):
            try:
                ...
                if len(analyses) != len(batch):
                    logger.warning(
                        f"Batch {batch_idx}: se esperaban {
                            len(batch)} análisis, "
                        f"se recibieron {len(analyses)}"
                    )

                logger.info(f"Batch {batch_idx} analizado con key {
                            client_idx} ({len(analyses)} noticias)")
                return analyses
                ...
                if e.status_code in (429, 503):
                    ...
                    continue
                logger.error(f"Error API con key {
                             client_idx} (status {e.status_code}): {e}")
                continue
            except Exception as e:
                last_error = e
        ...
        logger.error(f"Batch {batch_idx} falló tras {
                     len(self.clients)} intentos: {last_error}")
        return None
    ...
    async def analyze_and_save_news(
            ...
            for item, ad in zip(batch_news, analyses):
                try:
                    url = (item.get("source_url") or item.get("link")
                           or ad.get("fuente_url", "") or "")[:250]
                    tags = [t for t in ad.get(
                        "etiquetas", ["Servicios básicos"]) if t in self.ALLOWED_CATEGORIES]
                    if not tags:
                    ...
                    if url not in existing_urls:
                        content = item.get(
                            "content_text", "") or ad.get("resumen", "")
                        news_to_insert.append({
                            "title": ad.get("titulo", item.get("title", "Sin título")),
                            "summary": ad.get("resumen", ""),
                            "content_text": content,
                            "source_url": url,
                            "published_at": item.get("published_at") or datetime.now(),
                            "impact_level": urgency,
                            "affects": ad.get("impacto_personal", "")[:100] if ad.get("impacto_personal") else None,
        ...
        # Map URLs to IDs
        url_to_id = {url: nid for nid, url in db_session.query(
            News.news_id, News.source_url).all()}

        # Tags
        if tags_to_insert:
            unique_tags = {t["name"]: t for t in tags_to_insert}.values()
            existing_tags = {r[0]: r[1] for r in db_session.query(
                NewsTag.name, NewsTag.tag_id).all()}
            new_tags = [t for t in unique_tags if t["name"]
            ...
            existing_pairs = {(r[0], r[1]) for r in db_session.query(
                NewsTagMap.news_id, NewsTagMap.tag_id).all()}
            tag_map_bulk = []
            seen_pairs = set()
            for url, info in news_db_map.items():
                ...
                for tname in info.get("tags", []):
                    ...
                    if tid and (nid, tid) not in seen_pairs and (nid, tid) not in existing_pairs:
        ...
        if analyses_bulk:
            logger.info(
                f"Insertando {len(analyses_bulk)} análisis personalizados")
        ...
        logger.info(f"Análisis completado: {
                    len(saved_results)} noticias procesadas")
    ...
    async def get_all_analyzed_news(self, user_id: str, db_session) -> list[dict]:
        ...
        for pa in results:
            ...
            try:
                ad = json.loads(pa.analysis_text) if isinstance(
                    pa.analysis_text, str) else (pa.analysis_text or {})
            except Exception:
